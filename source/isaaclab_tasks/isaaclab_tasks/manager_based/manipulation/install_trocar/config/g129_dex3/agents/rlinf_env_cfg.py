...
from rlinf.envs.isaaclab.tasks.install_trocar import mdp

from rlinf.envs.isaaclab.tasks.install_trocar.config import G1RobotPresets, CameraPresets  # isort: skip
...
@configclass
class InstallTrocarSceneCfg(InteractiveSceneCfg):
    """Scene configuration for the install_trocar task (robot + objects + lights)."""
    ...
    scene = AssetBaseCfg(
        ...
        spawn=UsdFileCfg(
            usd_path=f"{USD_ROOT}/assets/scene03.usd",
        ),
    )
    ...
    trocar_1 = RigidObjectCfg(
        ...
        spawn=UsdFileCfg(
            usd_path=f"{USD_ROOT}/assets/Assets/Trocar002/Trocar002-xform-wo.usd",
    ...
    box = ArticulationCfg(
        prim_path="/World/envs/env_.*/surgical_box",
        spawn=UsdFileCfg(
            usd_path=f"{USD_ROOT}/assets/Assets/Box001/Box001.usd",
        ),
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=joint_names, scale=1.0, use_default_offset=False, offset=offset_dict, preserve_order=True
    )
...
@configclass
class G1InstallTrocarEnvCfg(ManagerBasedRLEnvCfg):
    """Unitree G1 robot install trocar environment configuration class
    inherits from ManagerBasedRLEnvCfg, defines all configuration parameters for the entire environment
    """

    # scene settings
    scene: InstallTrocarSceneCfg = InstallTrocarSceneCfg(
        num_envs=1,
        env_spacing=6.0,
...
@configclass
class EventCfgFixBoxRotation(EventCfg):
...
@configclass
class G1InstallTrocarEvalEnvCfg(G1InstallTrocarEnvCfg):
    """Eval-friendly env cfg.
    """

    # Override events to enforce deterministic per-env box yaw on every reset.
    events: EventCfgFixBoxRotation = EventCfgFixBoxRotation()
