...
class G1DreamDojoActionBridge:
    ...
    def __init__(self, statistics_path: str | Path, device: torch.device | str):
        ...
        self.minimum = torch.tensor(statistics["min"], dtype=torch.float32, device=self.device)
        self.maximum = torch.tensor(statistics["max"], dtype=torch.float32, device=self.device)
    ...
    def encode_30hz_actions(self, actions_30hz: torch.Tensor, baseline_state: torch.Tensor) -> torch.Tensor:
        ...
        if actions_30hz.ndim != 3 or actions_30hz.shape[1] != 25:
            raise ValueError(f"Expected 25 raw actions shaped [B,25,28] or [B,25,43], got {tuple(actions_30hz.shape)}")
        ...
        if actions_30hz.shape[-1] == 28:
            ...
            raise ValueError(f"Expected 28-D or 43-D actions, got {actions_30hz.shape[-1]}")

        sampled = self._min_max(actions_43, self.minimum, self.maximum)[:, ::2]
        blocks = []
        for start in range(0, 12, 4):
            blocks.append(sampled[:, start + 1 : start + 5] - sampled[:, start : start + 1])
        deltas = torch.cat(blocks, dim=1)
        encoded = deltas.new_zeros(deltas.shape[0], deltas.shape[1], DREAMDOJO_ACTION_DIM)
        ...
        normalized_state = self._min_max(baseline_state, self.state_minimum, self.state_maximum)
    ...
    def encode_policy_chunk(
        ...
        if actions_28.ndim != 3 or actions_28.shape[1:] != (12, 28):
            raise ValueError(f"Expected policy actions [B,12,28], got {tuple(actions_28.shape)}")
        if previous_action_28.shape != (actions_28.shape[0], 28):
            raise ValueError(f"Expected previous actions [B,28], got {tuple(previous_action_28.shape)}")
