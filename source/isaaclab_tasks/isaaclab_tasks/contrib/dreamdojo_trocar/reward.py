...
class _MilestoneNet(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        backbone.fc = nn.Sequential(nn.Dropout(0.2), nn.Linear(backbone.fc.in_features, NUM_HEADS))
...
class BatchedMilestoneReward:
    ...
    def __init__(
        ...
        *,
        ...
        self.mean = torch.tensor([0.485, 0.456, 0.406], device=self.device).repeat(len(FRAME_OFFSETS)).view(1, -1, 1, 1)
        self.std = torch.tensor([0.229, 0.224, 0.225], device=self.device).repeat(len(FRAME_OFFSETS)).view(1, -1, 1, 1)
        self.reset()

    def reset(self, env_mask: torch.Tensor | None = None) -> None:
        if not hasattr(self, "frame_history"):
            self.frame_history = [deque(maxlen=max(FRAME_OFFSETS) + 1) for _ in range(self.num_envs)]
            self.recent = [[deque(maxlen=w) for w in CONFIRM_WINDOW] for _ in range(self.num_envs)]
            self.stage = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        indices = range(self.num_envs) if env_mask is None else env_mask.nonzero(as_tuple=False).flatten().tolist()
    ...
    def _prepare(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        crops = [
            cv2.resize(frame, (STORE_SIZE[1], STORE_SIZE[0]), interpolation=cv2.INTER_AREA)[
                y0 : y0 + CROP_SIZE[0], x0 : x0 + CROP_SIZE[1]
            ]
            for frame in arrays
        ]
        return torch.from_numpy(np.stack(crops)).permute(0, 3, 1, 2).to(self.device, dtype=torch.float32).div(255)
    ...
    def _predict(self, frame_rgb: torch.Tensor) -> torch.Tensor:
        ...
        for env_index in range(self.num_envs):
            ...
            stacks.append(
                torch.cat(
                    [newest_first[min(offset, len(newest_first) - 1)] for offset in FRAME_OFFSETS],
                    dim=0,
                )
            )
    ...
    def advance(self, probs: torch.Tensor) -> torch.Tensor:
        ...
        for env_index in range(self.num_envs):
            ...
            for head in range(NUM_HEADS):
                self.recent[env_index][head].append(bool(probs[env_index, head] >= THRESHOLDS[head]))
            stage = before
            while stage < NUM_HEADS and sum(self.recent[env_index][stage]) >= CONFIRM_COUNT[stage]:
                stage += 1
    ...
    @torch.no_grad()
    def score_chunk(self, frames_rgb: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Score `[B,T,3,H,W]` 15-fps frames and return rewards/probabilities."""
        if frames_rgb.ndim != 5 or frames_rgb.shape[0] != self.num_envs:
            raise ValueError(f"Expected [B,T,3,H,W] for B={self.num_envs}, got {tuple(frames_rgb.shape)}")
        chunk_rewards = torch.zeros(self.num_envs, frames_rgb.shape[1], device=self.device)
        chunk_probs = torch.zeros(self.num_envs, frames_rgb.shape[1], NUM_HEADS, device=self.device)
